...
class Solution:
    def combinationSum(self,candidates,target,index,temp,result):
        ...
        self.combinationSum(candidates,target-candidates[index],index+1,temp,result)
        
        # Backtrack: Remove the last added element and explore the next possibility (not pick case)
        temp.pop()
        self.combinationSum(candidates,target,index+1,temp,result)
        
                    

result = set()
sol = Solution()
sol.combinationSum([10,1,2,7,6,1,5],8,0,[],result)
